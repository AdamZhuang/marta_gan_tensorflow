...
class MartaGan:
  ...
  def build_model(self):
    ...
    self.real_images = tf.placeholder(tf.float32, [None, self.image_size, self.image_size, self.image_dim],
                                      name='real_images')
    self.random_real_images = tf.placeholder(tf.float32, [None, self.image_size, self.image_size, self.image_dim],
                                             name='random_real_images')
    # c, real and pair
    ...
    # c, real but not pair
    _, self.output_real_but_not_pair_confidence, _, _ = MartaGanBasicNetWork.discriminator(
      input_c=self.input_code,
      input_x=self.random_real_images,
      class_num=self.class_num,
      is_train=True,
      reuse=True)
    ...
    self.g_loss, self.g_optimizer = self._generator(self.output_confidence_fake.outputs,
                                                    self.output_pair_confidence_fake.outputs,
                                                    self.feature_real,
                                                    self.feature_fake,
    ...
    self.d_loss, self.d_optimizer = self._discriminator(self.output_pair_confidence_real,
                                                        ...
                                                        self.output_real_but_not_pair_confidence,
  ...
  def train(self, image_label_dict, batch_size=64, epoch=1000, load_epoch=0):
    with tf.Session() as sess:
      ...
      for cur_epoch in range(load_epoch + 1, epoch):
        ...
        batched_random_file_names = data_utils.get_shuffled_batch_file_names(image_label_dict, batch_size)
        for cur_batch in range(total_batch):
          ...
          batch_images_real, batch_labels_real = data_utils.get_one_batch_data(cur_batch_file_names, image_label_dict,
                                                                               self.class_num)
          batch_random_images_real = data_utils.get_one_batch_data_without_labels(batched_random_file_names[cur_batch])
          ...
          d_loss, _ = sess.run([self.d_loss, self.d_optimizer],
                               feed_dict={self.input_code: batch_labels_real,
                                          ...
                                          self.random_real_images: batch_random_images_real})
          # train g
          for _ in range(1):
  ...
  def _generator(self, output_confidence, output_pair_confidence, feature_real, feature_fake, style_features_fake, style_features_real):
    """
    ...
    """
    # loss of generator
    g_loss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_confidence, labels=tf.ones_like(output_confidence))) + \
              tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_pair_confidence, labels=tf.ones_like(output_pair_confidence)))
    ...
    g_loss = g_loss1 + 0.2 * g_loss2 + 5 * g_loss3

    # optimizer of generator
    g_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="generator")
    # self.net_g.print_params(False)
    g_optimizer = tf.train.AdamOptimizer(self.g_learning_rate, beta1=self.beta1).minimize(g_loss, var_list=g_vars)

    return g_loss, g_optimizer

  def _discriminator(self, output_pair_confidence_real, output_pair_confidence_fake, output_real_but_not_pair_confidence,
    ...
    d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_confidence_real.outputs,
                                                                         labels=tf.ones_like(output_confidence_real.outputs)))  # real == 1
    d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_confidence_fake.outputs,
                                                                         labels=tf.zeros_like(output_confidence_fake.outputs)))  # fake == 0
    d_loss1 = d_loss_real + d_loss_fake



    d_loss_pair_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_pair_confidence_real.outputs,
                                                                              labels=tf.ones_like(output_pair_confidence_real.outputs)))  # real == 1
    # to hard so remove
    d_loss_pair_fake_1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_pair_confidence_fake.outputs,
                                                                                labels=tf.zeros_like(output_pair_confidence_fake.outputs)))  # fake == 0
    d_loss_pair_fake_2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output_real_but_not_pair_confidence.outputs,
                                                                                labels=tf.zeros_like(output_real_but_not_pair_confidence.outputs)))  # fake == 0
    # d_loss2 = d_loss_pair_fake_1 + d_loss_pair_fake_2 + d_loss_pair_real
    d_loss2 = d_loss_pair_fake_2 + d_loss_pair_real
    ...
    d_optimizer = tf.train.AdamOptimizer(self.d_learning_rate, beta1=self.beta1).minimize(d_loss, var_list=d_vars)
